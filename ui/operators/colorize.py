...
class ColorizeObjectsFromCollections(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.msg = _("Assigned {} unique colors.").format(len(collectiondict))
...
class ColorizeObjectsFromGroups(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.active_object and context.active_object.M4.is_group_empty:
            return _("Recursively Set Random Object Viewport Colors for Group Objects based on Group Membership\nALT: Set Group Member Colors based on existing Group Empy Colors\nCTRL: Only Colorize the Active Group")
        else:
            return _("Recursively Set Random Object Viewport Colors for Group Objects based on Group Membership\nALT: Set Group Member Colors based on existing Group Empy Colors")
    ...
    def execute(self, context):
        ...
        active_group = context.active_object if context.active_object and context.active_object.M4.is_group_empty else None
    ...
    def colorize_group(self, empty, recursive=False):
        children = [c for c in empty.children if c.M4.is_group_object]
