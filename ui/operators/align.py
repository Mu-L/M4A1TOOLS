    ...
    def invoke(self, context, event):
        ...
        if self.mode == 'VIEW':
            ...
            if self.type in ['ZERO', 'AVERAGE', 'CURSOR'] and self.direction in ['HORIZONTAL', 'VERTICAL']:
                ...
                popup_message(_("You can't combine {} with {}!".format(self.type,self.direction)), title=_("Invalid Property Combination"))
                return {'CANCELLED'}
...
class CenterEditMesh(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if event.alt and event.ctrl:
            popup_message("Hold down ATL, CTRL or neither, not both!", title=_("Invalid Modifier Keys"))
            return {'CANCELLED'}
