...
class SaveVersionedStartupFile(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if os.path.exists(startup_path):
            ...
            self.report({'INFO'}, 'Versioned Startup File saved: {}'.format(biggest_idx + 1))
...
class ScreenCast(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        ...
        if screencast_keys:
            return _("Screen Cast recent Operators and Keys")
        return _("Screen Cast Recent Operators")
    ...
    def execute(self, context):
        ...
        wm = context.window_manager
        setattr(wm, 'M3_screen_cast', not wm.M4_screen_cast)

        is_casting = wm.M4_screen_cast
