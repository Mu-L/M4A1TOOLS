...
class ToggleWireframe(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.mode == 'OBJECT':
            return _("Toggle Wireframe display for the selected objects\nNothing Selected: Toggle Wireframe Overlay, affecting all objects")
        elif context.mode == 'EDIT_MESH':
            return _("Toggle X-Ray, resembling how edit mode wireframes worked in Blender 2.79")
    ...
    def execute(self, context):
        ...
        if context.mode == "OBJECT":
            ...
            context.scene.M4.show_edit_mesh_wire = not context.scene.M4.show_edit_mesh_wire
