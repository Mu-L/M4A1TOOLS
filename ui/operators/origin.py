...
class OriginToActive(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.mode == 'OBJECT':
            return _("Set Selected Objects' Origin to Active Object\nALT: only set Origin Location\nCTRL: only set Origin Rotation")
        elif context.mode == 'EDIT_MESH':
            return _("Set Mesh Object's Origin to Active Vert/Edge/Face\nALT: only set Origin Location\nCTRL: only set Origin Rotation")
...
class OriginToCursor(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.mode == 'OBJECT':
            return _("Set Selected Objects' Origin to Cursor\nALT: only set Origin Location\nCTRL: only set Origin Rotation")
        elif context.mode == 'EDIT_MESH':
            return _("Set Mesh Object's Origin to Cursor\nALT: only set Origin Location\nCTRL: only set Origin Rotation")
