...
class ViewAxis(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        ...
        if context.mode == 'OBJECT':
            ...
            selection = _('Verts') if tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (True, False, False) else _('Edges') if tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (False, True, False) else _('Faces') if tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (False, False, True) else _('Elements')
        else:
            selection = _('Elements')

        if m3.custom_views_local:
            return _("Align Custom View to Active Object\nALT: Align View to Active %s") % (selection)
        if m3.custom_views_cursor:
            return _("Align Custom View to Cursor\nALT: Align View to Active %s") % (selection)
        else:
            return _("Align View to World\nALT: Align View to Active to Active %s") % (selection)
    ...
    def invoke(self, context, event):
        ...
        if event.alt:
            ...
            if not mx:
                context.scene.M4.custom_views_local = False
...
class ToggleOrbitMethod(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.preferences.inputs.view_rotate_method == 'TURNTABLE':
            return _("Change Orbit Method from Turntable to Trackball")
        return _("Change Orbit Method from Trackball to Turntable")
...
class ToggleOrbitSelection(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.preferences.inputs.use_rotate_around_active:
            return _("Disable Orbit around Selection")
        return _("Enable Orbit around Selection")
