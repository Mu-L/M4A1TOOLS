...
class CursorToOrigin(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if not context.space_data.overlay.show_cursor and not context.scene.M4.draw_cursor_axes:
...
class CursorToSelected(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        if context.mode == 'OBJECT':
            return _("Align Cursor with Selected Object(s)\nALT: Only set Cursor Location\nCTRL: Only set Cursor Rotation")

        elif context.mode == 'EDIT_MESH':
            return _("Align Cursor with Vert/Edge/Face\nALT: Only set Cursor Location\nCTRL: Only set Cursor Rotation")
    ...
    def invoke(self, context, event):
        ...
        if context.mode == 'OBJECT' and active and (not sel or active.M4.is_group_empty):
...
class SelectedToCursor(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if context.active_object and context.active_object.M4.is_group_empty and context.active_object.children:
