...
class SimpleDeformHelperToolPanel(Panel, GizmoUtils):
    ...
    def poll(cls, context):
        show_in_tool_options = GizmoUtils.pref_().sdh_show_gizmo_property_location == 'ToolOptions'
    ...
    def draw_property(layout, context):
        if GizmoUtils.poll_simple_deform_public(context):
            ...
            layout.prop(ctrl_obj,
                        ...
                        text='')
            layout.prop(pref,
                        'sdh_update_deform_wireframe',
                        icon='MOD_WIREFRAME',
                        text='')
            layout.prop(pref,
                        'sdh_show_set_axis_button',
                        icon='EMPTY_AXIS',
                        text='')
            if pref.modifier_deform_method_is_bend:
                layout.prop(pref,
                            'sdh_display_bend_axis_switch_gizmo',
                            toggle=1)
            layout.prop(pref,
                        'sdh_modifiers_limits_tolerance',
                        text='')

    def draw_settings(self, context):
        show_in_settings = GizmoUtils.pref_().sdh_show_gizmo_property_location == 'ToolSettings'
        if show_in_settings:
