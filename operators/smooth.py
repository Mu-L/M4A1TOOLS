...
class ToggleSmooth(bpy.types.Operator):
    ...
    def toggle_korean_bevel(self, context, obj, toggle_type='TOGGLE'):
        ...
        if bpy.app.version >= (4, 1, 0):
            is_smooth = obj.M4.has_smoothed or bm.faces[0].smooth

        else:
            is_smooth = obj.M4.has_smoothed or (degrees(angle) == 180 and fm.faces[0].smooth)

        if not is_smooth:
            if toggle_type in ['TOGGLE', 'ENABLE']:

                if bpy.app.version < (4, 1, 0):
                    obj.M4.smooth_angle = angle
            ...
            if toggle_type in ['TOGGLE', 'DISABLE']:
                ...
                if bpy.app.version < (4, 1, 0):
                    obj.data.auto_smooth_angle = obj.M4.smooth_angle
