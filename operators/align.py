...
class Align(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.mode in ['ORIGIN', 'CURSOR', 'FLOOR']:
            ...
            if active and active.M4.is_group_empty and active.children:
                sel = [active]

        elif self.mode in 'ACTIVE':
            all_empties = [obj for obj in sel if obj.M4.is_group_empty and obj != active]
...
class AlignRelative(bpy.types.Operator):
    ...
    def regroup(self, dup_data, target, dup, debug=False):
        if target.M4.is_group_object and target.parent and target.parent.M4.is_group_empty:
            if (dup.M4.is_group_object and self.active.M4.is_group_object) and (dup.parent and self.active.parent) and (dup.parent.M4.is_group_empty and self.active.parent.M4.is_group_empty) and (dup.parent == self.active.parent):
                if debug:
