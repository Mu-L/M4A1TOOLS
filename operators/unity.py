...
class PrepareExport(bpy.types.Operator):
    ...
    def poll(cls, context):
        return not [obj for obj in context.visible_objects if obj.M4.unity_exported]

    @classmethod
    def description(cls, context, properties):
        if context.scene.M4.unity_export:
    ...
    def execute(self, context):
        ...
        path = context.scene.M4.unity_export_path
        triangulate = context.scene.M4.unity_triangulate
        export = context.scene.M4.unity_export
    ...
    def prepare_for_export(self, obj, sel, matrices, bone_children, triangulate=False, depth=0, child=False):
        def prepare_object(obj, mx, depth, child):
            ...
            obj.M4.unity_exported = True

            mx = matrices[obj]
            obj.M4.pre_unity_export_mx = flatten_matrix(mx)
        ...
        def prepare_mesh(obj, depth):
            obj.M4.pre_unity_export_mesh = obj.data
        ...
        def prepare_armature(obj, depth):
            obj.M4.pre_unity_export_armature = obj.data
            obj.data = obj.data.copy()
...
class RestoreExport(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return [obj for obj in context.visible_objects if obj.M4.unity_exported]

    def execute(self, context):
        print("\nINFO: Restoring Pre-Unity-Export Status")

        detriangulate = context.scene.M4.unity_triangulate

        exported = [obj for obj in context.visible_objects if obj.M4.unity_exported]
        meshes = []
        armatures = []
    ...
    def restore_exported(self, obj, exported, bone_children, meshes, armatures, detriangulate=True, depth=0, child=False):
        def restore_object(obj, depth, child):
            ...
            obj.matrix_world = obj.M4.pre_unity_export_mx
            obj.M4.pre_unity_export_mx = flatten_matrix(Matrix())
            obj.M4.unity_exported = False
        ...
        def restore_mesh(obj, depth, meshes):
            ...
            obj.data = obj.M4.pre_unity_export_mesh
            obj.M4.pre_unity_export_mesh = None
        ...
        def restore_armature(obj, depth, armatures):
            ...
            obj.data = obj.M4.pre_unity_export_armature
            obj.M4.pre_unity_export_armature = None
        ...
        if obj in exported:
            ...
            if obj in bone_children:
                ...
                if obj.type == 'MESH' and obj.M4.pre_unity_export_mesh:
                    restore_mesh(obj, depth, meshes)

                elif obj.type == 'ARMATURE' and obj.M4.pre_unity_export_armature:
